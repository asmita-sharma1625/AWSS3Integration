...
class S3NotificationMonitor:
  ...
  def __init__(self):
    ...
    if s3NotifyMsg is not None:
      ...
      self.getS3Object(objectKey, NEW_CONFIG_PATH)
      ...
      self.notifyChange(oldConfig, newConfig)
  ...
  def notifyChange(self, oldConfig, newConfig):
    ...
    if configChangeDetector.compareConfig():
      ...
      #call mail server to send notification with diff 
    
  def applyChange(self, newConfig, node, path):
    if self.getResponseMail() is not None:
